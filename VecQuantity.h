...
struct VecQuantity {
    Vec<n, T> value;
    VecQuantity (Vec<n, T> v) : value(v) {}
    VecQuantity () : value(0) {}
    VecQuantity& operator=(T v) { value = v; }
    operator std::string() {
        Unit<l, m, t> u;
        std::ostringstream s;
        s << value;
...
template<int l1, int m1, int t1, int l2, int m2, int t2, unsigned int n, class T>
Quantity<l1+l2, m1+m2, t1+t2, T> operator*(VecQuantity<l1, m1, t1, n, T> q1, VecQuantity<l2, m2, t2, n, T> q2)
{
